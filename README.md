...
ウェーブをひとつ越えるごとに、描画と音が 1 段階「進化」します（`src/game/fidelity.js`）。

| Wave | ティア | 描画 | 音 |
| --- | --- | --- | --- |
| 1 | 8-BIT | 192px相当の低解像度・16色パレット・走査線 | 矩形波＋ビットクラッシュ、モノラル |
| 2 | 16-BIT | 解像度と色数（32色）が向上 | フィルタ追加、わずかな残響 |
| 3 | VGA | テクスチャ精細化・階調表現（減色は6段） | ステレオ定位、2層合成 |
| 4 | SVGA | 床をテクスチャ描画、動的ライティング、接地影 | 残響とサブベース |
| 5 | HD | 高解像度・柔らかな陰影・ブルーム | 3層合成の厚い音 |
| 6+ | ULTRA | 最高精細・環境光・強めのブルーム | 最も深い残響と低音 |

同じレイキャスティング描画のまま、オフスクリーンバッファの解像度・アセットの粗さ・
減色・走査線・ライティング・後処理を切り替えることで実現しています。
フレーム時間を監視して解像度を自動で上下させるため、非力な端末でも動きます。

### 武器のレベルアップ

敵を倒すと武器 XP が貯まり、6 段階まで強化されます（強化コアを拾うと加速）。

| Lv | 武器 | 強化内容 |
| --- | --- | --- |
| 1 | ブラスター Mk.I | 標準装備 |
| 2 | ブラスター Mk.II | 威力と連射速度が向上 |
| 3 | デュアルボルト Mk.III | 2連射（拡散） |
| 4 | パルスランス Mk.IV | 貫通弾（1体）|
| 5 | ヴォイドレイ Mk.V | 3連射・貫通強化 |
| 6 | アニヒレーター Mk.VI | 最大出力・貫通3体 |

レベルごとに武器スプライトそのものが描き分けられ、銃口の閃光も大きくなります。
...
| `src/game/fidelity.js` | 描画・音の進化ティアと武器レベルの定義 |
