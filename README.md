...
WebGL2 による 3D 描画をゲーム開始時から使います（`src/game/renderer3d.js`）。
外部ライブラリもモデルデータも使わず、すべてコードで組み立てています。

- **ジオメトリ**: 迷路の格子から壁・床・天井のポリゴンを生成。見えない面は省き、
  角には簡易アンビエントオクルージョンを頂点に焼き込む。さらに木箱・ドラム缶・
  配管・天井照明を自動配置して通路に変化をつける（照明は実際の光源になる）
- **マテリアル**: `materials.js` がレンガ・金属パネル・コンクリート・床などの
  アルベドと高さマップを生成し、高さから法線マップを作る（粗さはアルファに格納）
- **ライティング**: GGX 近似の物理ベースシェーディング。プレイヤーのヘッドライトに
  加え、銃口の閃光・敵弾・アイテムが点光源として周囲を照らす
- **影**: 光源へ向かって迷路グリッドをシェーダ内でレイマーチし、遮蔽を判定する
- **後処理**: HDR で描画 → 明部抽出 → ガウスぼかし → ACES トーンマッピング、
  周辺減光、色収差、粒状ノイズ
