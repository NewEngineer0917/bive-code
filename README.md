...
音と 3D モデルは**実ファイルのアセット**としてリポジトリに入っています。
どちらも生成スクリプトから作られており、手作りのアセットに差し替えられます。

| 場所 | 中身 | 生成 |
| --- | --- | --- |
| `public/audio/*.wav` | 効果音24種（銃声・リロード・被弾・環境音など） | `npm run make:audio` |
| `public/models/*.glb` | 3Dモデル10種（敵3種・車3種・信号機・小物） | `npm run make:models` |

- 効果音は `scripts/make-audio.js` が DSP で合成します。ノイズのトランジェント、
  低域の胴鳴り、機構音、残響を層で重ね、共振フィルタとサチュレーションを通して
  16bit WAV に焼き込んでいます。実行時は再生と空間表現（左右定位・距離減衰・
  遠距離のこもり・ピッチのゆらぎ）だけを行います。
- モデルは `scripts/make-models.js` が glTF バイナリ（.glb）として書き出します。
  パーツがノードに分かれており、`leg_l` / `leg_r` / `arm_l` / `arm_r` / `torso` /
  `head` という名前のノードは歩行アニメーションが自動で付きます。
- 単一HTML版をビルドすると、これらは base64 で埋め込まれ 1 ファイルで完結します。

#### 自作モデルに差し替える

1. Blender などで作る（1 ユニット = 1 メートル、Y 軸が上、-Z が前）
2. **glTF Binary (.glb)** で書き出す。パーツは上記の名前でノード分けする
3. `public/models/` に同じファイル名で置く（例 `enemy_gunner.glb`）
4. `npm run make:models` は実行しない（生成物を上書きしてしまうため）

現在のローダーが対応しているのは **頂点・法線・単色マテリアル**（baseColorFactor /
metallic / roughness / emissive）です。テクスチャ付きモデルやスケルタルアニメーション
（Mixamo など）を使う場合はローダーの拡張が必要です。
